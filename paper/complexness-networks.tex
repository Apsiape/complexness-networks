...
partition-relative notion. For \emph{frame resources} (one rebit
per holder), the frame twirl of Definition~\ref{def:walsh} gives a
quantitative handle: the twirled resource is free, so usefulness
certifies that some nontrivial frame character is doing work
...
classically correlated --- hence free --- state (for resources
with several rebits per holder this per-rebit twirl need not
produce a free state, which is why Definition~\ref{def:bound} and
Theorems~\ref{thm:activation}--\ref{thm:noirr} are stated for
frame-code resources). In a fixed assisted realization of a
...
$D^{a:b}_\Om(\rho)>0$.
(iii) Consequently, for $m = 2$ no useful resource has zero rate:
charge zero forces $\zeta = 0$, hence real separability by the
converse direction of \cite{cfr2001} --- the single load-bearing
external input of this clause --- hence freeness. For $m = 3$ the
...
weight, which is exactly what the per-pair dichotomy below turns
...
hence positive $(a,b)$ rate by Theorem~\ref{thm:w2}(ii). Neither
part alone suffices: the auxiliaries have zero $(a,b)$ rate
(their charge code contains no $(a,b)$ weight-2 word,
Theorem~\ref{thm:span}) and so does $\sigma_C$
($e_a+e_b\notin C$, Theorem~\ref{thm:distance}).
\end{theorem}
...
unit-neutral at best: for $|c| = 4$ the descent consumes one $\Om$
and yields one ($\tau_4$ acts as a relay), and for higher weights
it consumes more than it yields --- the content is
\emph{accessibility} of the resource's charge, not net gain;
quantitative rates remain open. Noncommutativity can make the
...
experimentally, arXiv:2502.20102.
