...
pair, in product), and in general the descent of a used
weight-$2k$ codeword consumes $k-1$ declared auxiliaries and
yields $k$ pairwise references, each auxiliary returned in kind:
the descent is catalytic over its auxiliaries.\footnote{Version
...
measurement branches at $|c|=4$ and $|c|=6$.} The content is
\emph{accessibility} of the resource's charge together with unit
net gain per descent; asymptotic conversion rates remain
